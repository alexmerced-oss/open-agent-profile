...
def main() -> int:
    ...
    cargo_locked = next(item["version"] for item in cargo_lock["package"] if item["name"] == "open-agent-profile")
    ...
    versions = {
        ...
        "Rust package": tomllib.loads((ROOT / "rust/Cargo.toml").read_text())["package"]["version"],
